...
for i, row in enumerate(params):
    ...
    setattr(process, 'pixelTracksClean' + str(i), cms.EDProducer( "TrackWithVertexSelector",
        src = cms.InputTag( 'pixelTracks' + str(i) ),
    ...
    setattr(process, 'pixelSeeds' + str(i), cms.EDProducer("SeedGeneratorFromProtoTracksEDProducer",
        InputCollection = cms.InputTag('pixelTracksClean' + str(i)),
    ...
    setattr(process, 'initialTrackCands' + str(i),cms.EDProducer("CkfTrackCandidateMaker",
        ...
        src = cms.InputTag('pixelSeeds' + str(i)),
        useHitsSplitting = cms.bool(False)
    ))

    setattr(process, 'initialTracks' + str(i), cms.EDProducer("TrackProducer",
        ...
        src = cms.InputTag('initialTrackCands' + str(i)),
        useHitsSplitting = cms.bool(False),
    ...
    setattr(process, 'simpleValidation' + str(i), cms.EDAnalyzer('SimpleValidation',
                ...
                ptMinTP = cms.double(0.8),
                signalOnlyTP = cms.bool(True),
                stableOnlyTP = cms.bool(False),
                tipTP = cms.double(3.5),
                trackLabels = cms.VInputTag('initialTracks' + str(i)),
...
taskListClean = [getattr(process, 'pixelTracksClean'+str(i)) for i in range(totalTasks)]
taskListSeeds = [getattr(process, 'pixelSeeds'+str(i)) for i in range(totalTasks)]
taskListTracksCands = [getattr(process, 'initialTrackCands'+str(i)) for i in range(totalTasks)]
taskListTracks = [getattr(process, 'initialTracks'+str(i)) for i in range(totalTasks)]
taskListVal = [getattr(process, 'simpleValidation'+str(i)) for i in range(totalTasks)]

# Tasks and sequences
process.pixelTracksTask = cms.Task(*taskListCUDA, *taskListSoA, *taskList, *taskListClean, *taskListSeeds, *taskListTracksCands, *taskListTracks)
...
# process.HLTTrackingV61Task = cms.Task(process.MeasurementTrackerEvent, process.generalTracks, process.initialStepSeeds, process.initialStepTrackCandidates, process.initialStepTrackCutClassifier, process.initialStepTrackSelectionHighPurity, process.initialStepTracks, process.pixelTracksClean, process.pixelTracksQuads, process.pixelVertices)
# process.HLTTrackingV61Sequence = cms.Sequence(process.HLTTrackingV61Task, process.pixelTracksTask, process.siPixelClustersTask, process.siPixelRecHitsTask, process.trackerClusterCheckTask)

# process.siPixelClustersTask = cms.Task(process.onlineBeamSpotToCUDA, process.siPixelClusters, process.siPixelClustersCUDA, process.siPixelClustersLegacy, process.siPixelDigisPhase2SoA)
# process.siPixelRecHitsTask = cms.Task(process.siPixelRecHits, process.siPixelRecHitsCPU, process.siPixelRecHitsCUDA, process.siPixelRecHitsSoA)


# process.raw2digi_step = cms.Path(process.RawToDigi_pixelOnly)
process.localPath = cms.Path(process.siPixelClustersTask, process.siPixelRecHitsTask)
