...
process.HLTIterativeTrackingWithoutDoubletRecovery = cms.Sequence( 
...
process.GlobalTag = GlobalTag(process.GlobalTag, '130X_mcRun3_2023_realistic_relvals2023C_v3', '')
...
for i, row in enumerate(params):
    ...
    setattr(process, 'hltPixelTracksGPU' + str(i), process.hltPixelTracksGPU.clone(
    ...
    taskList.append(getattr(process, 'hltPixelTracksGPU'+str(i)))

    for f in process.HLTIterativeTrackingWithoutDoubletRecovery.moduleNames():
        if isinstance(getattr(process,f), cms.EDProducer):
            new_name = f + str(i)
            setattr(process, new_name, getattr(process,f).clone()) 
...
process.raw2digi_step = cms.Path(process.HLTDoLocalPixelTask)#process.RawToDigi_pixelOnly)
